...
class Villager extends EventEmitter {
  ...
  listenTo(ticker) {
    ...
    this.ticker.on('tick', function(tick) {
      console.log(`------------------------------`);
      console.log(`${self.name}: I heard that it's ${tick.toLocaleString(DateTime.TIME_WITH_SECONDS)}`);
      console.log(`${self.name} ${self.completeAction()}`, `(${self.status()})`);
    });
  }

  status() {
    return `${this.action}:${this.wealth}c ${this.hunger}h`;
  }
